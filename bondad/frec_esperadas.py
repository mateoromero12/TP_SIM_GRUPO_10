...
def fe_normal(n, intervalos, parametros):
    ...
    for intervalo in intervalos:
        ...
        densidad = (math.e ** (-0.5 * ((marca_clase - media) / desviacion) ** 2)) / (
            desviacion * math.sqrt(2 * math.pi)
        )
...
def fe_exponencial(n, intervalos, parametros):
    ...
    lam_da = parametros[0] if isinstance(parametros, (tuple, list)) else parametros
    frecuencias = []
